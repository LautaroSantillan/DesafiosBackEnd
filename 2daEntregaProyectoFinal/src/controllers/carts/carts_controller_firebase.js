...
export const Carts_controller_firebase = class Carts_container {
	...
	get = async (cart_id) => {
		try {
			...
			return { error: "Error al traer carrito: " + err };
		}
	};
		...
		try {
			...
			if (product_index === -1) {
				...
				if (product.stock < cart_products[product_index].quantity + 1) {
					throw new Error("Producto sin stock suficiente");
				} else {
			...
			return { error: "Error añadiendo producto: " + err };
		}
	};
	...
	decrementProductQuantity = async (cart_id, product_id) => {
		try {
			...
			return { error: "Error al restar la cantidad del producto" + err };
		}
	};
	...
	deleteProduct = async (cart_id, product_id) => {
		try {
			...
			return { error: "Error al eliminar producto: " + err };
		}
	};
